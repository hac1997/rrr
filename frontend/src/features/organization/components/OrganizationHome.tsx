...
const OrganizationHome: React.FC<OrganizationHomeProps> = ({ onLogout }) => {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          <h2 className="text-xl font-bold text-gray-800">Organização não encontrada</h2>
          <p className="text-gray-600">Não foi possível carregar os dados da organização.</p>
          <button onClick={handleLogout} className="mt-4 px-4 py-2 bg-primary text-white rounded-lg">Sair</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-green-50">
      <div className="flex">
        {/* Sidebar */}
        <aside className="w-64 min-h-screen bg-white shadow-xl border-r-2 border-gray-200">
          <div className="p-6 border-b-2 border-gray-200 bg-gradient-to-br from-orange-50 to-green-50">
            <div className="flex items-center space-x-3 mb-4">
              <div className="w-14 h-14 bg-gradient-to-br from-primary to-secondary rounded-full flex items-center justify-center text-white font-bold text-2xl shadow-lg">
                {orgData.name.charAt(0)}
              </div>
              <div>
                <h3 className="font-bold text-gray-800 text-lg">{orgData.name}</h3>
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 mb-2 font-medium ${currentView === item.id
          ...
          <div className="absolute bottom-0 w-64 p-4 border-t-2 border-gray-200">
            <button
              onClick={onLogout}
