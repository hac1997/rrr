...
const EventFeed: React.FC<EventFeedProps> = ({ dashboardData }) => {
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto w-full">
      ...
      <div className="mb-6 bg-gradient-to-r from-orange-50 to-green-50 rounded-xl p-4 md:p-5 border-l-4 border-primary shadow-sm" role="region" aria-label="Notificações recentes">
        ...
        <div className="space-y-2 ml-0 md:ml-11">
          {notifications.map(notif => (
            <div key={notif.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-sm bg-white rounded-lg p-3 shadow-sm">
      ...
      <div className="mb-6">
        <div className="flex items-center mb-2">
          <TrendingUp className="h-6 w-6 md:h-7 md:w-7 text-primary mr-2" aria-hidden="true" />
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Feed de Eventos</h1>
        </div>
        <p className="text-gray-600 text-sm md:text-base ml-0 md:ml-9">Descubra oportunidades de voluntariado e acompanhe novidades</p>
      </div>
      ...
      <div className="space-y-6">
        {loading ? (
          ...
          feedItems.map(item => (
            <article key={item.id} className="bg-white rounded-xl shadow-lg border border-gray-200 relative hover:shadow-xl transition-shadow duration-300" aria-label={`Evento: ${item.title}`}>
              {item.image && (
              ...
              <div className="p-4 md:p-5">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
                  <div className="flex items-center space-x-3 min-w-0">
                    <div className="h-10 w-10 bg-primary rounded-full flex items-center justify-center text-white font-bold text-lg shrink-0" aria-hidden="true">
                      {item.author.charAt(0)}
                    </div>
                    <div className="min-w-0">
                      <h2 className="font-bold text-gray-800 text-base md:text-lg truncate">{item.title}</h2>
                  ...
                  <time className="text-xs text-gray-500 shrink-0">{item.date}</time>
                </div>

                <p className="text-gray-700 mb-4 text-sm md:text-base leading-relaxed">{item.content}</p>

                {(item.eventDate || item.location) && (
                  <div className="flex flex-col sm:flex-row sm:space-x-6 space-y-3 sm:space-y-0 mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    {item.eventDate && (
                      <div className="flex items-center" role="group" aria-label="Data do evento">
                        <div className="bg-blue-500 p-2 rounded-lg mr-2">
                          <Calendar className="h-4 w-4 text-white" />
                        </div>
                        <span className="text-gray-700 font-medium text-sm md:text-base">{item.eventDate}</span>
                      </div>
                    )}
                    {item.location && (
                      <div className="flex items-center" role="group" aria-label="Localização do evento">
                        <div className="bg-secondary p-2 rounded-lg mr-2">
                          <MapPin className="h-4 w-4 text-gray-800" />
                        </div>
                        <span className="text-gray-700 font-medium text-sm md:text-base">{item.location}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-t border-gray-200 pt-4">
                  <div className="flex space-x-4 sm:space-x-6" role="group" aria-label="Ações do evento">
                    <button
                      onClick={() => handleLike(item.id)}
                      className="flex items-center space-x-2 text-gray-600 hover:text-red-500 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 rounded p-1"
                      aria-label={`Curtir evento. ${item.likes} curtidas`}
                    >
                      <Heart className="h-5 w-5" aria-hidden="true" />
                      <span className="font-medium text-sm md:text-base">{item.likes}</span>
                    </button>
                    <button
                      onClick={() => handleComment(item.id)}
                      className="flex items-center space-x-2 text-gray-600 hover:text-primary transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded p-1"
                      aria-label={`Comentar evento. ${item.comments} comentários`}
                    >
                      <MessageCircle className="h-5 w-5" aria-hidden="true" />
                      <span className="font-medium text-sm md:text-base">{item.comments}</span>
                    </button>
                    <button
                  ...
                  <button
                    ...
                    aria-label="Salvar evento para mais tarde"
